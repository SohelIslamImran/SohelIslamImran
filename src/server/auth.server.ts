...
export type AuthenticationFailure = "missing_token" | "invalid_token" | "configuration_missing";
...
export async function authenticateAccess(
	...
	const localOwner = configuredOwnerEmail(env);
	if (env.ENVIRONMENT === "development" && localOwner && isLoopbackRequest(request)) {
		return {
			authenticated: true,
			identity: { email: localOwner, subject: "local-development" },
		};
	}
...
/** Throw a safe response at the owner-only server-function boundary. */
