...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  const description =
    "The small, useful internet trail of Sohel Islam Imran: work, writing, and ways to connect.";
    ...
    <article className="links-card" style={{ "--links-card-index": index } as React.CSSProperties}>
      <Link className="links-card__target" to={`/links/${link.id}`}>
        ...
        {(link.handle || link.description) && (
          <span className="links-card__detail">{link.handle ?? link.description}</span>
        )}
        {link.description && link.handle && <span className="links-card__description">{link.description}</span>}
        <span className="links-card__arrow" aria-hidden="true">↗</span>
      </Link>
      <button
        className="links-card__copy"
        type="button"
...
export default function Links({ loaderData }: Route.ComponentProps) {
  ...
  const links = [
    ...content.profileLinks.filter((link) => link.featured),
    ...content.profileLinks.filter((link) => !link.featured),
  ];
      ...
      <section className="links-page" aria-labelledby="links-title">
        ...
        <header className="links-intro">
          ...
          <p className="links-lede">
            {content.identity.name} — {content.identity.role}. A short list of places to read,
            browse, or start a conversation.
          </p>
        </header>

        <div className="links-list" aria-label="Profile links">
          {links.length > 0 ? (
