...
export function meta({ loaderData }: Route.MetaArgs) {
	...
	const description = `Case studies from ${name}, Lead Full Stack Engineer at Kuno: TypeScript and React product architecture, secure services, data systems, CI/CD, and open-source tooling.`;
	...
	return createSeoMeta({
		...
		pathname: '/work',
		content,
		image: '/images/social-work.png',
		imageAlt: 'Full-stack engineering work at Kuno',
		jsonLd: [
...
const systemDomains = [
	{
		...
		problem: 'Different roles need different context without losing a shared source of truth.',
		decision: 'Keep permissions explicit across the interface and service boundary.',
		boundary: 'TypeScript services · database state · guarded release checks',
	...
	},
	{
		...
		description: 'Programs, pathways, and people intersect. I turn those overlaps into flows that teams can test and people can follow.',
		problem: 'Programs, pathways, and people overlap in ways that are easy to misunderstand.',
		decision: 'Model the lifecycle as small, testable transitions with clear next actions.',
		boundary: 'Cohorts · pathways · matching rules · localized UI',
	...
	},
	{
		...
		problem: 'A feature is unfinished until it can reach a running environment safely.',
		decision: 'Treat CI, observability, and release permissions as part of the product surface.',
		boundary: 'CI/CD · CLI · environments · recovery paths',
...
function KunoOrbit() {
	...
	const initialDomain = systemDomains.findIndex((domain) => domain.id === searchParams.get('domain'));
	const [activeIndex, setActiveIndex] = useState(initialDomain >= 0 ? initialDomain : 0);
	...
	useEffect(() => {
		const next = systemDomains.findIndex((domain) => domain.id === searchParams.get('domain'));
		if (next >= 0 && next !== activeIndex) setActiveIndex(next);
	}, [activeIndex, searchParams]);

	const moveBy = (delta: number) => {
		selectDomain(normalizeIndex(activeIndex + delta));
	};
	...
	const selectDomain = (index: number) => {
		...
		nextParams.set('domain', systemDomains[nextIndex].id);
	...
	const handlePointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
		...
		if (event.pointerType === 'touch' && Math.abs(distanceX) <= Math.abs(distanceY)) return;
		if (Math.abs(distanceX) < 52) return;
		if (event.pointerType === 'touch') {
			event.currentTarget.setPointerCapture(event.pointerId);
			setIsDragging(true);
		}
	...
	return (
		<section className="worldline-lens" id="system-lens" aria-labelledby="system-lens-title" data-reveal>
			<div className="worldline-lens__heading">
				...
				<p>Drag the orbit or use the controls. Each point is a real Kuno concern, described at the level that can be shared publicly.</p>
			</div>
			...
			<div className="worldline-lens__body">
				...
				>
					...
					<div className="worldline-orbit" aria-hidden="true">
						...
						{systemDomains.map((domain, index) => {
							...
							return (
								<span
									className={`worldline-orbit__node worldline-orbit__node--${domain.color}${activeIndex === index ? ' is-active' : ''}`}
									key={domain.id}
									style={
										{
				...
				<div className="worldline-lens__copy">
					...
					<div
						...
						tabIndex={0}
					>
						<p className="worldline-kicker">
							{activeDomain.index} / {activeDomain.label}
						</p>
						<h3>{activeDomain.title}</h3>
						<p aria-live="polite">{activeDomain.description}</p>
						<dl className="worldline-lens__evidence">
							<div>
								<dt>Problem</dt>
								<dd>{activeDomain.problem}</dd>
							</div>
							<div>
								<dt>Decision</dt>
								<dd>{activeDomain.decision}</dd>
							</div>
							<div>
								<dt>Boundary</dt>
								<dd>{activeDomain.boundary}</dd>
							</div>
						</dl>
		...
		<article className="worldline-role" data-reveal>
			...
			<div className="worldline-role__content">
				<div className="worldline-role__meta">
					<time dateTime={dateTimeFromPeriod(role.period)}>{role.period}</time>
...
export default function Work({ loaderData }: Route.ComponentProps) {
	...
	const kunoRoles = content.experience.filter((experience) => experience.company.toLowerCase() === 'kuno').slice();
	const earlierRoles = content.experience.filter((experience) => experience.company.toLowerCase() !== 'kuno');
	...
	const avatarSrc = avatar ? `/media/${encodeURIComponent(avatar.id)}` : '/images/sohel-linkedin.png';
			...
			<div className="worldline-page">
				<section className="worldline-hero" aria-labelledby="work-title">
					...
					<div className="worldline-hero__grid">
						<div className="worldline-hero__copy">
							<p className="worldline-kicker">{content.identity.role} · Kuno</p>
							<h1 id="work-title">Product systems that give complexity a clear next step.</h1>
							<p className="worldline-hero__intro">
								I lead full-stack product work at Kuno, connecting product rules, secure services, and delivery systems so people can focus
				...
				<section className="worldline-kuno" id="kuno-work" aria-labelledby="kuno-title" data-reveal>
					<div className="worldline-section-intro">
						...
						<p>
							Kuno builds professional-development products for organizations. My role sits across the product, the services underneath it,
							and the path that gets each change into a running environment.
						</p>
					</div>
					...
					{kunoRoles.length > 0 ? (
						<div className="worldline-role-list">
							{kunoRoles.map((role) => (
								<KunoRole key={role.id} role={role} />
							))}
						</div>
					) : (
				...
				<section className="worldline-open-source" id="open-source" aria-labelledby="open-source-title" data-reveal>
					<div className="worldline-section-intro worldline-section-intro--compact">
						...
						<p>
							These projects are the smaller, visible edge of the same practice: make one problem easier to understand, then keep the
							boundary honest.
						</p>
					</div>
				...
				<section className="worldline-close" aria-labelledby="worldline-close-title" data-reveal>
					...
					<p>
						Interface, service, data, release. I like work that asks me to stay with an outcome long enough to make the edges make sense.
					</p>
					<div className="worldline-actions">
						<ArrowLink to="/story">Read the story</ArrowLink>
						<ArrowLink to={`mailto:${content.contact.email || content.identity.email}`} external>
